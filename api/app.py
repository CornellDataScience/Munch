...
import os
import sys
ml_directory_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'ml'))
sys.path.append(ml_directory_path)

from clip_test import classify_img
...
driver = GraphDatabase.driver(
    ...
    auth=(os.environ.get("DATABASE_USERNAME"), os.environ.get("DATABASE_PASSWORD")),
)
...
def populate_db(df1, df2):
    ...
    for _, r in df1.iterrows():
        ...
        """
            ...
            """
        query = f"MERGE(i:Ingredient {{name: '{ingredient.lower()}', protein: '{protein}', carbs: '{carbs}', fats: '{fats}'}})"

        # run query
        session.run(query)

    for recipe in list(df2.columns)[1:]:
        session.run(f"MERGE(r:Recipe {{name:'{recipe.lower()}'}})")
